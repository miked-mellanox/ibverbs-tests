...
struct ibv_pd *domain = NULL;
struct ibv_cq *queue[ENDPOINTS] = { NULL };
struct ibv_qp *queue_pair[ENDPOINTS] = { NULL };
struct ibv_mr *region[ENDPOINTS] = { NULL };
char buff[ENDPOINTS][BUFF_SIZE];
...
void xmit(int i, ibv_wr_opcode op) {
...
void to_rtr(int i, uint32_t remote_qpn, uint16_t dlid) {
	struct ibv_qp_attr attr;
	int flags;
